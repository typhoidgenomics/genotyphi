...
```
python genotyphi.py --bam *.bam --ref_id AL513382 --ref AL513382.fasta
```

## Basic Usage - own VCF

```
python genotyphi.py --vcf *.vcf --ref_id AL513382 --ref AL513382.fasta
```

### Required options

```
--bam (or --vcf)   Path to one or more BAM or VCF files, generated by mapping to the Typhi CT18 reference genome (AL513382)
...
--ref_id           The name of the reference genome used for mapping, must match the header in the reference
                   sequence fasta file (supplied by --ref).
...
--output                Specify the location of the output text file.

--samtools_location     Specify the location of the folder containing the samtools installation if not standard/in path.

--bcftools_location     Specify the location of the folder containing the bcftools installation if not standard/in path.

--vcf_parsnp            Specify this in the place of --vcf or --bam for multi-sample vcf files.
