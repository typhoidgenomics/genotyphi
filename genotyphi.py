...
def parse_args():
    ...
    parser.add_argument('--samtools_location', type=str, default='',
                    help='Location of folder containing samtools installation if not standard/in path.')
    parser.add_argument('--bcftools_location', type=str, default='',
...
loci = [655112, 773487, 1804415, 1840727, 3640678, 270120, 102135, 316489, 4105384, 555826, 2360997, 4664137, 2166082,
        ...
        3996717, 2640029, 989024, 3806278, 1611156, 2348902]
...
def main():
    ...
    if args.bam:
        ...
        for locus in ordered_loci:
            temp_bed_file.write(args.ref_id + '\t' + str(locus -1) + '\t' + str(locus) + '\n')  # write bed file from matrix
        ...
        for bam in args.bam:
            ...
            run_command([args.samtools_location + 'samtools', 'mpileup', '-q', str(args.phred), '-ugB', '-f', args.ref_fasta, '-l', temp_bed_file.name, bam, '-o', bam[:-4] + '.output', '-I'])
            run_command([args.bcftools_location + 'bcftools', 'call', '-c', bam[:-4] + '.output', '-o', bam[:-4] + '.vcf'])
    ...
    if args.vcf:
        for vcf in args.vcf:
            ...
            if any_ref_line > 0:
                ...
                output_file.write(vcf + '\tNo SNPs encountered against expected reference. Wrong reference or no SNP calls?\n')
